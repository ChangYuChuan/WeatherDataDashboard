...
export class AppComponent {
  ...
  private async monthForecast_old(
    startDate: Date
  ): Promise<ISingleHumidTempData[]> {
    let today = new Date();
    let result: ISingleHumidTempData[] = [];
    ...
    for (let i = 0; i <= 30; i = i + 4) {
      ...
      if (fourDateData.length == 0) {
        ...
        fourDateData = (await this.weatherSrv.GetFourDaystWeatherData(date))
          .items;
        temp = temp - 1;
      }
  ...
  private async monthForecastOneByOne(
    startDate: Date
  ): Promise<ISingleHumidTempData[]> {
    let today = new Date();
    let result: ISingleHumidTempData[] = [];
   ...
   */
  private transformToHumidTemp(
    input: IForecastWeatherData[]
  ): ISingleHumidTempData[] {
    if (input == undefined || input.length == 0) return [];
    let result: ISingleHumidTempData[] = [];

    input[0].forecasts.forEach((element) => {
